...
trait UserPokemonTable {
  ...
  case class UserPokemonRow(userPokemonId: Int, userId: Int, pokemonId: Int, combatPower: Int, ivAttack: Int, ivDefense: Int, ivHitPoints: Int, minLevel: Int, maxLevel: Int)
  /** GetResult implicit for fetching UserPokemonRow objects using plain SQL queries */
  implicit def GetResultUserPokemonRow(implicit e0: GR[Int]): GR[UserPokemonRow] = GR{
    prs => import prs._
    UserPokemonRow.tupled((<<[Int], <<[Int], <<[Int], <<[Int], <<[Int], <<[Int], <<[Int], <<[Int], <<[Int]))
  }
  ...
  class UserPokemon(_tableTag: Tag) extends profile.api.Table[UserPokemonRow](_tableTag, Some("goraid"), "UserPokemon") {
    def * = (userPokemonId, userId, pokemonId, combatPower, ivAttack, ivDefense, ivHitPoints, minLevel, maxLevel) <> (UserPokemonRow.tupled, UserPokemonRow.unapply)
    /** Maps whole row to an option. Useful for outer joins. */
    def ? = ((Rep.Some(userPokemonId), Rep.Some(userId), Rep.Some(pokemonId), Rep.Some(combatPower), Rep.Some(ivAttack), Rep.Some(ivDefense), Rep.Some(ivHitPoints), Rep.Some(minLevel), Rep.Some(maxLevel))).shaped.<>({r=>import r._; _1.map(_=> UserPokemonRow.tupled((_1.get, _2.get, _3.get, _4.get, _5.get, _6.get, _7.get, _8.get, _9.get)))}, (_:Any) =>  throw new Exception("Inserting into ? projection not supported."))
