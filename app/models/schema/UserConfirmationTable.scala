...
trait UserConfirmationTable {
  ...
  case class UserConfirmationRow(userConfirmationId: Int, token: String, expiration: java.sql.Timestamp, userId: Int)
  ...
  implicit def GetResultUserConfirmationRow(implicit e0: GR[Int], e1: GR[String], e2: GR[java.sql.Timestamp]): GR[UserConfirmationRow] = GR{
    prs => import prs._
    UserConfirmationRow.tupled((<<[Int], <<[String], <<[java.sql.Timestamp], <<[Int]))
  }
  /** Table description of table UserConfirmation. Objects of this class serve as prototypes for rows in queries. */
  class UserConfirmation(_tableTag: Tag) extends profile.api.Table[UserConfirmationRow](_tableTag, Some("goraid"), "UserConfirmation") {
    def * = (userConfirmationId, token, expiration, userId) <> (UserConfirmationRow.tupled, UserConfirmationRow.unapply)
    /** Maps whole row to an option. Useful for outer joins. */
    def ? = ((Rep.Some(userConfirmationId), Rep.Some(token), Rep.Some(expiration), Rep.Some(userId))).shaped.<>({r=>import r._; _1.map(_=> UserConfirmationRow.tupled((_1.get, _2.get, _3.get, _4.get)))}, (_:Any) =>  throw new Exception("Inserting into ? projection not supported."))
